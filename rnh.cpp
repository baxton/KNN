...
class Ray {
    auto_ptr<RayImpl> pimpl;
public:
    Ray() {
        pimpl = auto_ptr<RayImpl>(new RayImpl());
    }
...
struct heap {
    enum {
        ...
        ARRAY_SIZE_BYTES = MAX_N * sizeof(rect),
    };
    int begin;
    int end;
    int count;

    T raw_heap[HEAP_SIZE] __attribute__((aligned(128)));
...
struct AutoObj {
    ...
    void alloc() {oh = global_heap.allocate();}
    void free() {
        if (NOTHING != oh) {
...
private:
    AutoObj(const AutoObj&);
    AutoObj& operator= (const AutoObj&);
};
